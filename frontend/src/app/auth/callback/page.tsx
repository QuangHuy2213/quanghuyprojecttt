...
function CallbackContent() {
  ...
  const completeGoogleProfile = async () => {
    if (!/^0\d{9}$/.test(phone))
      return setError('Số điện thoại phải gồm 10 số và bắt đầu bằng 0.');
  ...
  if (needsPhone)
    return (
      <div className="w-[min(92vw,430px)] rounded-3xl bg-white p-8 shadow-2xl border">
        <div className="text-4xl">📱</div>
        <h1 className="mt-4 text-2xl font-black text-slate-900">Bổ sung số điện thoại</h1>
        <p className="mt-2 text-sm text-slate-500">
          Thông tin này là bắt buộc để xác nhận giao dịch và bảo vệ tài khoản đăng nhập
          bằng Google.
        </p>
        <input
          autoFocus
          ...
          className="mt-5 w-full rounded-xl border px-4 py-3 text-slate-900"
        />
        {error && <p className="mt-2 text-sm font-bold text-rose-600">{error}</p>}
        <button
          onClick={completeGoogleProfile}
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <svg
        className="animate-spin h-10 w-10 text-[#1877F2]"
        fill="none"
        viewBox="0 0 24 24"
      >
        <circle
      ...
      <span className="text-[#1877F2] font-bold animate-pulse text-lg tracking-wide">
...
export default function AuthCallbackPage() {
  return (
    <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center">
      ...
      <Suspense
        fallback={
          <div className="text-[#1877F2] font-bold animate-pulse">
            Đang tải cấu hình...
          </div>
        }
      >

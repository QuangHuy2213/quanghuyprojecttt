...
// Đường dẫn ảnh Avatar mặc định (Giống ảnh bạn cung cấp)
...
export default function ChatPage() {
  ...
  // 1. Kiểm tra đăng nhập và Lấy danh sách chat đã lưu
  useEffect(() => {
  ...
  // 2. Xử lý khi được điều hướng từ trang Chi tiết (Tạo cuộc hội thoại mới)
  useEffect(() => {
    if (sellerId && sellerName && currentUser) {
      ...
      setChatList((prevList) => {
        ...
        // Nếu đã từng chat, lấy thông tin cũ
        if (isExist) {
          setActiveChat(isExist);
          return prevList;
        }

        // Nếu chưa từng chat, tạo mới người này
        const newChatTarget = {
          id: sellerId,
          name: sellerName,
          avatar: DEFAULT_AVATAR, // Dùng avatar mặc định
          lastMessage: 'Bắt đầu cuộc trò chuyện...',
          time: new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })
        };

        const updatedList = [newChatTarget, ...prevList];
        // Lưu lại danh sách người đã chat
  ...
  // 3. Khi bấm vào 1 người, tải toàn bộ tin nhắn của người đó lên
  useEffect(() => {
    if (currentUser && activeChat) {
      // Mỗi người bán sẽ có 1 "phòng chat" lưu trữ riêng trong LocalStorage
  ...
  // 5. Hàm xử lý Gửi tin nhắn và Lưu lại
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    // Lưu tin nhắn vào phòng chat riêng
    const roomKey = `chat_messages_${currentUser.id}_${activeChat.id}`;
    localStorage.setItem(roomKey, JSON.stringify(updatedMessages));

    // Cập nhật lại tin nhắn cuối cùng ở Cột trái (Danh sách Chat)
    setChatList((prevList) => {
      ...
      // Đưa người vừa nhắn lên đầu danh sách

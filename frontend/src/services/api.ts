...
export const apiUrl = (path: string) => {
  ...
  const baseUrl =
    process.env.NEXT_PUBLIC_API_URL || 'https://quanghuy-security.onrender.com/api';
...
const rateLimitGroup = (path: string, method = 'GET') => {
  ...
  if (['/payment', '/checkout'].some((part) => normalized.includes(part)))
    return 'payment';
  if (['/upload', '/media', '/files'].some((part) => normalized.includes(part)))
    return 'upload';
...
export const getApiRetryDelay = (path: string, method = 'GET') => {
  ...
  try {
    ...
    /* Memory cooldown still works when storage is unavailable. */
  }
...
const recordRateLimit = async (response: Response, path: string, method: string) => {
  ...
  for (const group of groups) {
    ...
    try {
      existing = Math.max(existing, Number(localStorage.getItem(key)) || 0);
    } catch {
      /* Storage is optional. */
    }
...
export const apiFetch = async (path: string, options: RequestInit = {}) => {
  ...
  if (method !== 'GET' || typeof window === 'undefined')
    return perform(options.signal ?? undefined);
  ...
  if (!entry) {
    ...
    const promise = perform(controller.signal)
      .then(async (response) => {
        ...
        if (response.ok && version === cacheVersion && !controller.signal.aborted) {
          ...
          const ttl =
            pathname === 'cities' || pathname.startsWith('districts/') ? 300_000 : 1000;
